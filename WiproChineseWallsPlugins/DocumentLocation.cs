...
namespace ChineseWallsPlugins
{
    public class DocumentLocationHelper
    {
        ...
        public SharePointDocumentLocation CreateDocumentLocation(SharePointSite site, SharePointDocumentLocation docLoc, List<KeyValuePair<string, string>> userPermissionSet, ITracingService tracingService)
        {
            ...
            _spService.CreateFolder(site.AbsoluteURL, folderPathString, userPermissionSet, tracingService);
            docLoc.AbsoluteURL = folderPathString; // Set temporarily in memory to allow the caller to get the full path
            return docLoc;
        }

        public string ApplyFolderPermissions(string siteUrl, string relativePath, List<KeyValuePair<string, string>> userPermissionSet, ITracingService tracingService)
        {
            try
            {
                _spService.GrantFolderPermissions(siteUrl, relativePath, userPermissionSet, tracingService);
                return "Success!";
            }
            catch(Exception ex)
            {
                return String.Concat(ex.Message, Environment.NewLine, ex.StackTrace);
            }
        }

        public SharePointDocumentLocation CreateDocumentLocation(SharePointSite site, string documentLibraryName, wipro_deal regardingRecord, List<KeyValuePair<string, string>> userPermissionSet, ITracingService tracingService)
        {       
            if (regardingRecord != null && string.IsNullOrEmpty(regardingRecord.wipro_name))
            ...
            var existingDocumentLocation = this.GetDocumentLocationForRecordBySite(site, regardingRecord.ToEntityReference());
            ...
            docLoc.Name = regardingRecord.wipro_name;
            docLoc.RelativeUrl = GetFolderName(regardingRecord.wipro_name);
            docLoc.RegardingObjectId = regardingRecord.ToEntityReference();
        ...
        {
            return GetFolderName(logicalName);
        }

        private string GetFolderName(string recordName)
        {
            var name = recordName;
